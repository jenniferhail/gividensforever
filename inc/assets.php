...
function thegividens_assets_header() {
  ...
  $file = 'x-header';
  wp_enqueue_script( 'thegividens-header', get_stylesheet_directory_uri() . '/js/' . $file . '.js', $deps = array('jquery'), filemtime( get_template_directory() . '/js/' . $file . '.js' ), false );

  // Register and enqueue our main stylesheet with versioning based on last modified time
  wp_register_style( 'thegividens-style', get_stylesheet_uri(), $dependencies = array(), filemtime( get_template_directory() . '/style.css' ) );
  wp_enqueue_style( 'thegividens-style' );
}
add_action( 'wp_enqueue_scripts', 'thegividens_assets_header' );



// Load footer assets; a more complex example of a smooth asset-loading approach for WordPress themes
function thegividens_assets_footer() {

  // Initialize variables
  $name = 'thegividens-footer';   // This is the script handle
  ...
  if ( thegividens_SCRIPTS_PAGELOAD && ( is_archive() || is_home() || is_search() ) ) {
...
add_action( 'wp_enqueue_scripts', 'thegividens_assets_singular' );
